...
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in 3B05_concatenatedfastafile.txt -dbtype nucl -out se3B05GenomeDatabaseEpi'
#Input CDS file
CDS<- readDNAStringSet("se3B05.ffn", use.names = TRUE)
RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task megablast -query se3B05.ffn -db se3B05GenomeDatabaseEpi -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 1 -perc_identity 100 -out se3B05GenesMatch.blast'
...
system('sort -u -f -k1,1 se3B05GenesMatch.blast > se3B05singleBLASThit.txt') #select only one top hit from hits
```
...
```{r}
TopHits<-fread("se3B05singleBLASThit.txt") #Reading in blast output
