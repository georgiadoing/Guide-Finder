...
Make sure to modify commands to match most updated version of BLAST (2.15.0 as of July 2023).
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in E_coli_CP017100.txt -dbtype nucl -out GenomeDatabase'

RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'

ScrambleBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query BLASTscrambles.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out MatchScrambles.blast'
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("/projects/oh-lab/sierra/Guide-Finder/GuideFinder-Mac/E_coli_CP017100.txt")
...
CoordinatesDF<-fread("/projects/oh-lab/sierra/Guide-Finder/GuideFinder-Mac/DraftGenome_NewCoordinates.csv")
