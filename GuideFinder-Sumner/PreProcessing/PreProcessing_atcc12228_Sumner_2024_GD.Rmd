...
knitr::opts_knit$set(root.dir = '/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/PreProcessing')
setwd ('/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/PreProcessing')
...
so this chunk of code concatenates the contigs and adds a series of Ns between the contigs. Substitute the file name for the name of your file. 
...
```{r, eval=FALSE, echo=FALSE}
# S. epi genome
fastafile<-read.fasta(file="../ncbi_genomes/ncbi_dataset/data/GCF_000007645.1/GCF_000007645.1_ASM764v1_genomic.fna", as.string = TRUE) #read in fasta file
...
write.fasta(SingleSeqFasta,"SingleSequenceFastaFile",file="atcc12228_concatenatedfastafile.txt") #write out to fasta file 
```
...
InputGenome<-readDNAStringSet("atcc12228_concatenatedfastafile.txt", use.names = TRUE)
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in atcc12228_concatenatedfastafile.txt -dbtype nucl -out atcc12228GenomeDatabaseEpi'
#Input CDS file
CDS<- readDNAStringSet("../ncbi_genomes/ncbi_dataset/data/GCF_000007645.1/cds_from_genomic.fna", use.names = TRUE)
RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task megablast -query ../ncbi_genomes/ncbi_dataset/data/GCF_000007645.1/cds_from_genomic.fna -db atcc12228GenomeDatabaseEpi -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 1 -perc_identity 100 -out atcc12228GenesMatch.blast'
...
system('sort -u -f -k1,1 atcc12228GenesMatch.blast > atcc12228singleBLASThit.txt') #select only one top hit from hits
```
...
```{r}
TopHits<-fread("atcc12228singleBLASThit.txt") #Reading in blast output
...
write.csv(CorrectedCoordinates, file = "atcc12228_NewCoordinates.csv", row.names = F )
```
