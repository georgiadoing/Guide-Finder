...
```{r, eval=FALSE, echo=FALSE}
# S. epi genome
fastafile<-read.fasta(file="../staph_cluster_genomes/Gff/sepc7/sepc7.fna", as.string = TRUE) #read in fasta file
...
write.fasta(SingleSeqFasta,"SingleSequenceFastaFile",file="sepic7_concatenatedfastafile.txt") #write out to fasta file 
```
...
InputGenome<-readDNAStringSet("sepic7_concatenatedfastafile.txt", use.names = TRUE)
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in sepic7_concatenatedfastafile.txt -dbtype nucl -out sepic7GenomeDatabaseEpi'
#Input CDS file
CDS<- readDNAStringSet("../staph_cluster_genomes/Gff/sepc7/sepc7.ffn", use.names = TRUE)
RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task megablast -query ../staph_cluster_genomes/Gff/sepc7/sepc7.ffn -db sepic7GenomeDatabaseEpi -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 1 -perc_identity 100 -out sepic7GenesMatch.blast'
...
system('sort -u -f -k1,1 sepic7GenesMatch.blast > sepic7singleBLASThit.txt') #select only one top hit from hits
```
...
```{r}
TopHits<-fread("sepic7singleBLASThit.txt") #Reading in blast output
...
write.csv(CorrectedCoordinates, file = "sepic7_NewCoordinates.csv", row.names = F )
```
