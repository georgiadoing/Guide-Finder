...
```{r}
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in 3B05_concatenatedfastafile.txt -dbtype nucl -out se3B05GenomeDatabase'

RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query BLASTguides.fasta -db se3B05GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out se3B05MatchGuides.blast'

ScrambleBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query BLASTscrambles.fasta -db se3B05GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out se3B05MatchScrambles.blast'
...
```{r,tidy=TRUE}
Re_RunBlast<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db se3B05GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'
#If selecting option to relax off-target searches 
Re_RunBlast_Lowered<-'~/ncbi-blast-2.14.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db se3B05GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'
```
...
Matches<-fread("se3B05MatchGuides.blast")
...
Matches<-fread("se3B05MatchScrambles.blast")
Matchesdf<-data.frame(Matches$V1)
colnames(Matchesdf)<-"GuideID"
