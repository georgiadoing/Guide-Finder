...
```{r setup}
knitr::opts_knit$set(root.dir = '/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/GuideFinder_run/sepic7')
setwd ('/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/GuideFinder_run/sepic7')
```
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in ../../PreProcessing/sepic7_concatenatedfastafile.txt -dbtype nucl -out sepic7GenomeDatabase'

RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query sepic7BLASTguides.fasta -db sepic7GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out sepic7MatchGuides.blast'

ScrambleBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query BLASTscrambles.fasta -db sepic7GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out sepic7MatchScrambles.blast'
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("../../PreProcessing/sepic7_concatenatedfastafile.txt")
...
CoordinatesDF<-fread("../../PreProcessing/sepic7_NewCoordinates.csv")
...
```{r,tidy=TRUE}
Re_RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query re_sepic7BLASTguides.fasta -db sepic7GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_sepic7MatchGuides.blast'
#If selecting option to relax off-target searches 
Re_RunBlast_Lowered<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query re_sepic7BLASTguides.fasta -db sepic7GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_sepic7MatchGuides.blast'
```
...
writeXStringSet(ShortGuides, filepath = "sepic7BLASTguides.fasta") 
...
writeXStringSet(ScrambleGuides, filepath = "sepic7BLASTscrambles.fasta")
...
Matches<-fread("sepic7MatchScrambles.blast")
Matchesdf<-data.frame(Matches$V1)
colnames(Matchesdf)<-"GuideID"
...
writeXStringSet(Re_ShortGuidestoMatch, filepath = "re_sepic7BLASTguides.fasta") #write out as fasta file

system(MakeDatabase)
if (LowerOffTarget == TRUE) 
  {system(Re_RunBlast_Lowered)} else {system(Re_RunBlast)}

Re_Matches<-fread("re_sepic7MatchGuides.blast")
