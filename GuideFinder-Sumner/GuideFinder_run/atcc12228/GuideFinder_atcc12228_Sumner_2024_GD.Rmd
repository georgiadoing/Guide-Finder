...
```{r setup}
knitr::opts_knit$set(root.dir = '/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/GuideFinder_run/atcc12228')
setwd ('/projects/oh-lab/Georgia/Guide-Finder/GuideFinder-Sumner/GuideFinder_run/atcc12228')
```
...
BiocManager::install(c("Biostrings", "genbankr","annotate","stringr","data.table","seqinr","ape","refGenome","rentrez","reshape2","dplyr","purrr","stringi","GenomicFeatures","VariantAnnotation","rtracklayer"))

options(max.print=999999)

```

had to download an archived version of refgenome: https://cran.r-project.org/src/contrib/Archive/refGenome/refGenome_1.7.7.tar.gz

```{r, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
install.packages("doBy")
install.packages("~/Downloads/refGenome_1.7.7.tar.gz", repos = NULL, type = "source")
install.packages("~/Downloads/genbankr_1.26.0.tar.gz", repos = NULL, type = "source")
...
```{r}
MakeDatabase<-'~/ncbi-blast-2.15.0+/bin/makeblastdb -in ../../PreProcessing/atcc12228_concatenatedfastafile.txt -dbtype nucl -out atcc12228GenomeDatabase'

RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query atcc12228BLASTguides.fasta -db atcc12228GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out atcc12228MatchGuides.blast'

ScrambleBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query atcc12228BLASTscrambles.fasta -db atcc12228GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 11 -perc_identity .9 -out atcc12228MatchScrambles.blast'
...
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("../../PreProcessing/atcc12228_concatenatedfastafile.txt")

#GenomeName <- "3B05_NNN"
GenomeName <- "atcc12228"
...
GenBankInfo<- readGenBank(GBAccession("AE015929.1")) 
...
CoordinatesDF<-fread("../../PreProcessing/atcc12228_NewCoordinates.csv")
...
```{r,tidy=TRUE}
Re_RunBlast<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query re_atcc12228BLASTguides.fasta -db atcc12228GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_atcc12228MatchGuides.blast'
#If selecting option to relax off-target searches 
Re_RunBlast_Lowered<-'~/ncbi-blast-2.15.0+/bin/blastn -task blastn-short -query re_atcc12228BLASTguides.fasta -db atcc12228GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_atcc12228MatchGuides.blast'
```
...
writeXStringSet(ShortGuides, filepath = "atcc12228BLASTguides.fasta") 
...
Matches<-fread(paste0(GenomeName,"MatchGuides.blast"))
...
writeXStringSet(ScrambleGuides, filepath = "atcc12228BLASTscrambles.fasta")
...
Matches<-fread("atcc12228MatchScrambles.blast")
Matchesdf<-data.frame(Matches$V1)
colnames(Matchesdf)<-"GuideID"
...
writeXStringSet(Re_ShortGuidestoMatch, filepath = "re_atcc12228BLASTguides.fasta") #write out as fasta file

system(MakeDatabase)
if (LowerOffTarget == TRUE) 
  {system(Re_RunBlast_Lowered)} else {system(Re_RunBlast)}

Re_Matches<-fread("re_atcc12228MatchGuides.blast")
...
#write.csv(GenesAfterRefiltering, file=paste0(GenomeName, "InfoForGuidesUsingLowerThresholds.csv"))
